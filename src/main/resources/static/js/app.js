...
const HomePage = {
  ...
  `,
  components: { Hero, LoadingState, ErrorState, GlobalFloatingButtons },
  data() {
...
const SeatSelectionPage = {
        ...
        <template v-else>
          ...
          <div class="seat-selection">
            ...
            <div class="selection-summary">
              ...
              <button type="button" :disabled="selectedSeats.length === 0">確認訂票</button>
            </div>
          </div>
        </template>
      </div>
  ...
  `,
  components: { LoadingState, ErrorState },
  data() { return { movie: null, details: null, loading: true, error: null, selectedSeats: [] }; },
  computed: {
    seatRows() {
  ...
  methods: {
    async loadSeatData() {
      ...
      try {
        ...
        if (!showtimeRes.ok) throw new Error('Failed to load showtime');
    ...
    toggleSeat(seatId, reserved) {
      ...
      else this.selectedSeats.push(seatId);
    },
    getSeatClass(seat) {
      const classes = ['seat'];
